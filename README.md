...
- **支持历史对话与对象记忆**：连续对话中可复用历史上下文及已绑定客户或商机；对象不唯一时通过 Markdown 候选列表提示用户补充完整名称。
...
当前 Chatflow 由 47 个节点、69 条边组成，通过任务规划、证据调度和 Agent Router 按需启用不同能力；简单问题可跳过知识检索、外部情报或专项 Agent 等非必要链路。
