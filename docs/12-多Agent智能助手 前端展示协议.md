...
| 文档版本 | v1.1 |
| 更新时间 | 2026-07-18 |
| 当前状态 | 已按当前前端实现和后端协议校准 |
| 主要读者 | 前端、AI 应用开发、测试、后续协作 Agent |

本文档定义当前前端内部消费的字段、展示规则和禁止展示内容。前后端与 Chatflow 的完整协议以 `docs/06-多Agent智能助手 Chatflow 与前端协议规范.md` 为准；判断过程业务语义以 `docs/05-多Agent智能助手 判断过程事件规范.md` 为准。

## 1. 目标

本协议用于在开发前固定展示层边界，避免后续并行开发时出现以下偏差：

- Markdown 组件从结构化 payload 中自行解析业务对象、兼容状态或工具结果。
...
| `reply.writeback` | session state / adapter | 历史写回状态兼容 | 不直接渲染 payload |
...
- 承载普通问答、商机总结、风险分析、下一步建议、多结果澄清和内容草稿。
- 使用 Markdown 表达标题、段落、列表、表格、引用、代码块和图表 fence。

禁止：

- 不从 `answerText` 中解析客户、商机、候选对象、兼容 payload 或过程事件。
...
旧类型 `task_identified`、`object_selected`、`context_loaded`、`memory_used`、`rule_checked`、`risk_found` 和 `suggestion_generated` 只用于兼容历史会话。当前实时事件使用上表类型；未知 `type` 直接过滤。
...
| 字段 | 可选值 | 含义 | 消费位置 |
| --- | --- | --- | --- |
| `turn.status` | `default` / `generating` / `failed` | 整个 AI 回合状态 | `ConversationTurn` |
| `process.events[].status` | `running` / `completed` / `warning` / `failed` | 单个判断过程事件状态 | `ProcessTimeline` |
| `writeback.status` | `awaiting_confirm` / `confirmed` / `cancelled` / `failed` | 历史写回状态兼容 | hook / session state |

禁止把这三类 status 混用。例如：

- 不能用 `writeback.status = failed` 直接让整个 `turn.status` 变成失败。
- 不能用 `turn.status = generating` 替代 `process.events[].status = running`。
- 不能用 `process.events[].status = completed` 推断任何 CRM 写入已经发生。
...
Markdown 组件只负责把 `turn.text` 渲染得稳定、清晰、安全，不解释业务结构。
...
组件不得新增对 `processEvents`、`writeback`、`boundObject` 的依赖。
...
跟进记录、跟进计划和沟通内容草稿由 Markdown 渲染，不做成特殊表单或卡片。
...
判断过程模块不展示“工具调用本身”，只展示工具调用背后的业务过程。
...
| --- | --- | --- |
| `task_understanding` | 正在理解本轮任务 | 已理解本轮任务 |
| `attachment_analysis` | 正在解析附件内容 | 已解析附件内容 |
| `crm_retrieval` | 正在读取 CRM 资料 | 已读取 CRM 资料 |
| `knowledge_retrieval` | 正在检索业务规则 | 已检索业务规则 |
| `external_research` | 正在查询公开资料 | 已查询公开资料 |
| `sales_analysis` | 销售视角分析中 | 已完成销售视角分析 |
| `finance_analysis` | 财务视角分析中 | 已完成财务视角分析 |
| `delivery_analysis` | 交付视角分析中 | 已完成交付视角分析 |
| `legal_analysis` | 合同视角分析中 | 已完成合同视角分析 |
| `analytics_analysis` | 正在分析经营数据 | 已完成经营数据分析 |
| `answer_generation` | 正在整理回答 | 已生成回答 |
| `object_required` | 需要先确认本轮分析对象 | 需要先确认本轮分析对象 |
| `confirmation_required` | 历史兼容状态 | 历史兼容状态 |
| `writeback_completed` | 历史兼容状态 | 历史兼容状态 |
...
| `writeback_confirm` | `writeback-confirm` | 历史兼容展示 | 只恢复历史事件 | 只恢复历史状态 |
| `writeback_result` | `writeback-success` / `writeback-cancel` | 历史兼容展示 | 只恢复历史事件 | 只恢复历史状态 |
...
- smoke test 覆盖 Markdown source、过程事件、历史兼容字段、字段缺失和技术字段过滤。
