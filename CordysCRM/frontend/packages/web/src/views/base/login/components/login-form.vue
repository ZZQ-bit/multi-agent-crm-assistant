...
<script lang="ts" setup>
  ...
  import { prepareLoginPayload } from './loginCredentials';
    ...
    formRef.value?.validate(async (errors) => {
      if (!errors) {
        ...
        try {
          ...
          const payload = await prepareLoginPayload({
            username: userInfo.value.username,
            password: userInfo.value.password,
            authenticate: userInfo.value.authenticate,
            platform: 'WEB',
            loadPublicKey: async () => await appStore.initPublicKey(),
            encrypt: encrypted,
          });
          ...
          if (err instanceof Error && err.message === 'LOGIN_PUBLIC_KEY_UNAVAILABLE') {
    ...
    if (!props.isPreview) {
      try {
        if (!hasToken()) {
          preheat.value = false;
          clearToken();
        } else if (isLoginExpires()) {
          preheat.value = false;
          clearToken();
        } else {
          preheat.value = await userStore.isLogin(true);
          if (!preheat.value) {
            clearToken();
          }
        }
