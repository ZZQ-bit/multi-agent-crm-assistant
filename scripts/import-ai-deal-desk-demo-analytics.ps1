...
Write-Host "Importing 多Agent智能助手 analytics demo data into DB [$dbName] (Host ${hostName}:${port}, User $dbUser) ..."
...
if ($process.ExitCode -eq 0) {
    Write-Host "[OK] 多Agent智能助手 analytics demo data imported successfully."
    if ($output.Trim()) {
        Write-Host $output
    }
} else {
