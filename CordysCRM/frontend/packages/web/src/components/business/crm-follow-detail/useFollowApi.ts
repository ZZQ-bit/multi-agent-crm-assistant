...
export default function useFollowApi(followProps: {
  ...
  function transformField(item: FollowDetailItem, optionMap?: Record<string, any>) {
    ...
    const planStatus = String((item as CustomerFollowPlanListItem).status ?? '');
    const normalizedStatus =
      type.value === 'followPlan' && planStatus === 'PENDING'
    ...
    return {
      ...
      ...(type.value === 'followPlan' ? { status: normalizedStatus } : {}),
    };
  }
