...
// 路由白名单，无需校验权限与登录状态
export const WHITE_LIST = [NOT_FOUND, LOGIN_ROUTE];
...
export const featureRouteMap: Record<string, any> = {
  ...
  [AiDealDeskRouteEnum.AI_DEAL_DESK]: ModuleConfigEnum.AI_ASSISTANT,
...
export function findMenuRouteMapKey(routeName?: string | symbol | null, routeMap: Record<string, any> = featureRouteMap) {
  const routeNameText = routeName?.toString() ?? '';
  return Object.keys(routeMap)
    .sort((a, b) => b.length - a.length)
    .find((key) => routeNameText.includes(key));
}
